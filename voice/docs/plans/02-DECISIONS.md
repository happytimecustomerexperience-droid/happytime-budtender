...
- **⚠️ SUPERSEDED by ADR-023 (2026-06-23):** the owner chose a Vapi **Workflow** for the guided product questionnaire (reliability — node-level `variableExtractionPlan` over an LLM holding slots in a prompt). The Squad remains built + live as the fallback; the Workflow runs in parallel until cutover.
...
### ADR-023 — Guided product questionnaire runs as a Vapi Workflow (supersedes ADR-002 for that path)
- **Decision:** The product-suggestion flow (the owner's `happy-time-voice-agent-(full-script).json`) is built and provisioned as a **Vapi Workflow** — one `conversation` node per question, each with a `variableExtractionPlan` that captures the answer as a typed variable, and `tool` nodes that pass those variables to the existing webhook tools (`suggest_products`/`check_inventory`/`pair_upsell`/`notify_staff_issue`). It lives in `voice/workflow.py` (builder) + `provision_workflow` (deploy), separate from the squad provisioner. The Squad (ADR-002) stays built and **answers the live number** until the owner cuts the phone over to the workflow (A/B).
- **Rationale:** The owner's explicit, repeated priority is *reliably ask the exact questions AND retain + pass each answer to the budtender*. A Workflow's per-node variable extraction + Liquid pass-through is structurally more reliable for that than a single squad prompt holding every slot in the model's working memory. Vapi's June-2026 de-recommendation targets agents navigating **complex conditional edges**; this flow is a near-linear questionnaire (one 4-way category split), so that risk barely applies. The `/workflow` endpoint is beta/undocumented but its create shape was **pinned by a live smoke probe** (conversation nodes + `variableExtractionPlan` + `tool` nodes + `{type:"ai"}` edge conditions all accepted, 201). User instruction overrides the prior ADR (`CLAUDE.md` / global rule: session instructions win).
- **Consequences:** `core/services/vapi.py` gains `*_workflow` CRUD (the ADR-002 `_FORBIDDEN_PATH`/`_guard_path` guard is removed); `provision.py` stays workflow-free (a test asserts it). The workflow reuses the existing webhook + tool handlers untouched — no new backend/leak-guard/OTD code. Risk: a beta endpoint can change without notice; mitigated by keeping the squad as a one-PATCH fallback and a provisioning smoke check. Cutover = `PATCH /phone-number/{id} {workflowId, squadId:null}`; rollback = the reverse.
